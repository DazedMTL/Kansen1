...
//	var IndexVoice = 0;
	...
	var IndexQsave = 6;
	var IndexQload = 7;
	var IndexHistory = 8;	// LOGボタンのインデックス
	var IndexHide = 9;
...
//ボタン2列用		btn_y = 0;
		...
		for(var i = 0; i < count; i++)
		{
			...
			var xpos = btn_x + x;
//ボタン2列用			var ypos = btn_y + y;
			var obj;



			obj = backButtons[i];
//ボタン2列用			obj.setPos(xpos, ypos);
			obj.setPos(xpos, y);
...
//ボタン2列用			obj.setPos(xpos, ypos);
			...
			btn_x += obj.width+sf.sysbt_offset;		// オフセット無し。ボタン間を空ける時はこの項目を変更

//ボタン2列用			if(i == 5)
//ボタン2列用			{
//ボタン2列用			btn_y = y + obj.height;
//ボタン2列用			btn_x = 0;
//ボタン2列用			}
