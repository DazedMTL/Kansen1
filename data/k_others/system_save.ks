...
*save_loop1
	...
	[eval exp="tf.save_x[tf.save_i] = sf.thum_x + (144 + sf.thum_offset_x)*tf.save_column"]
	[eval exp="tf.save_y[tf.save_i] = sf.thum_y + (108 + sf.thum_offset_y)*tf.save_line"]
	; 上書き禁止チェックボックスを描画する座標を計算し、配列に格納
	[eval exp="tf.save_lock_x[tf.save_i] = sf.data_lock_x + (144 + sf.thum_offset_x)*tf.save_column"]
	[eval exp="tf.save_lock_y[tf.save_i] = sf.data_lock_y + (108 + sf.thum_offset_y)*tf.save_line"]
	; データ削除ボタンを描画する座標を計算し、配列に格納
	[eval exp="tf.save_erase_x[tf.save_i] = sf.data_erase_x + (144 + sf.thum_offset_x)*tf.save_column"]
	[eval exp="tf.save_erase_y[tf.save_i] = sf.data_erase_y + (108 + sf.thum_offset_y)*tf.save_line"]
...
[position layer="&tf.save_message1" page=fore left="&sf.data_num_x" top="&sf.data_num_y" width=800 height=100 frame="" color=0x000000 opacity=0 marginl=10 margint=0 marginr=0 marginb=0 draggable=false visible=true]
...
[image storage="bg_save" layer=base page=fore]
...
*save_loop00
	;セーブデータスロットのベース画像を描画。
	[pimage storage="savedata_base" layer=base page=fore visible=true dx="&tf.save_x[tf.save_i]" dy="&tf.save_y[tf.save_i]"]
...
;[if exp="kag.getBookMarkDate(0) != void || kag.getBookMarkDate(20) != void  || kag.getBookMarkDate(40) != void || kag.getBookMarkDate(60) != void || kag.getBookMarkDate(80) != void "]
	...
	[if exp="sf.new_savepage == sf.saveload_now_page "]
		[image storage="savenew" layer="&sf.newmarklayer" page=fore visible=true left="&tf.save_x[sf.new_saveclick]+0" top="&tf.save_y[sf.new_saveclick]+0" index=1004000]
	[endif]
;[endif]
...
;なんかこれおかしくね？
;	; カレントのメッセージレイヤを栞の日付・見出し表示用レイヤに変更
;	[current layer="&tf.save_message2" page=fore]
...
[if exp="tf.scene_mode == 0 && tf.gameplay == 1"]
	...
	[image left="&tf.sysjump_x[1]" top="&tf.sysjump_y[1]" storage="sysjump_save" layer=10 page=fore visible=true clipleft="&sf.sysjump_clipleft" cliptop="&sf.sysjump_cliptop" clipwidth="&sf.sysjump_clipwidth" clipheight="&sf.sysjump_clipheight" opacity=255]
[endif]
