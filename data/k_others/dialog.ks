...
[macro name=確認ダイアログ]
	...
	[if exp="tf.dialog_savedata_lock || tf.dialog_savedata_erase"]
		;こっちは削除ダイアログ用定義
		;3桁
		[if exp="tf.save_erase_num+sf.saveload_now_page*20+1 >= 100"]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 85"]
		;2桁
		[elsif exp="tf.save_erase_num+sf.saveload_now_page*20+1 >= 10"]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 90"]
		;1桁
		[else]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 95"]
		[endif]
	...
	[elsif exp="tf.dialog_save || tf.dialog_save_overwrite"]
		;こっちはセーブダイアログ用定義
		;3桁
		[if exp="tf.save_click_num+sf.saveload_now_page*20+1 >= 100"]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 85"]
		;2桁
		[elsif exp="tf.save_click_num+sf.saveload_now_page*20+1 >= 10"]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 90"]
		;1桁
		[else]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 95"]
	...
	[elsif exp="tf.dialog_load"]
		...
		;こっちはセーブダイアログ用定義
		;3桁
		[if exp="tf.load_click_num+sf.saveload_now_page*20+1 >= 100"]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 85"]
		;2桁
		[elsif exp="tf.load_click_num+sf.saveload_now_page*20+1 >= 10"]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 90"]
		;1桁
		[else]
			[eval exp="sf.erase_dialog_text2 = sf.erase_dialog_text + 95"]
	...
	[image storage="dialog_base" layer="&sf.layer_dialog_yesno"  page=fore visible=true left="&sf.dialog_x" top="&sf.dialog_y" index=1007000]
	;indexを選択肢の最大値より上に
	;x+472 y+281
	[ptext face="ＭＳ Ｐ明朝" layer="&sf.layer_dialog_yesno" page=fore x="&sf.dialog_caption_x" y="&sf.dialog_caption_y" size="&sf.dialog_text_size" color="&sf.dialog_text_color" shadow="&sf.dialog_text_shadow" edge="&sf.dialog_text_edge" bold="&sf.dialog_text_bold" text="確認"]


	;1行目
	[ptext face="ＭＳ Ｐ明朝" layer="&sf.layer_dialog_yesno" page=fore x="&sf.erase_dialog_text"     y="&sf.erase_dialog_text_y"    size="&sf.dialog_text_size" color="&sf.dialog_text_color" shadow="&sf.dialog_text_shadow" edge="&sf.dialog_text_edge" bold="&sf.dialog_text_bold" text="&tf.dialog_text01"]
	;以降はvoidでなければ表示	
	[if exp="tf.dialog_text02 != void"]
		[ptext face="ＭＳ Ｐ明朝" layer="&sf.layer_dialog_yesno" page=fore x="&sf.erase_dialog_text2"    y="&sf.erase_dialog_text_y"    size="&sf.dialog_text_size" color="&sf.dialog_text_color" shadow="&sf.dialog_text_shadow" edge="&sf.dialog_text_edge" bold="&sf.dialog_text_bold" text="&tf.dialog_text02"]
		[ptext face="ＭＳ Ｐ明朝" layer="&sf.layer_dialog_yesno" page=fore x="&sf.erase_dialog_text+120" y="&sf.erase_dialog_text_y"    size="&sf.dialog_text_size" color="&sf.dialog_text_color" shadow="&sf.dialog_text_shadow" edge="&sf.dialog_text_edge" bold="&sf.dialog_text_bold" text="&tf.dialog_text03"]
	[endif]
	;2行目
	[if exp="tf.dialog_text04 != void"]
		[ptext face="ＭＳ Ｐ明朝" layer="&sf.layer_dialog_yesno" page=fore x="&sf.erase_dialog_text"     y="&sf.erase_dialog_text_y+25" size="&sf.dialog_text_size" color="&sf.dialog_text_color" shadow="&sf.dialog_text_shadow" edge="&sf.dialog_text_edge" bold="&sf.dialog_text_bold" text="&tf.dialog_text04"]
	[endif]
[endmacro]
...
[elsif exp="tf.dialog_savedata_erase"]
	...
	[eval exp="tf.dialog_text02 = tf.save_erase_num+sf.saveload_now_page*20+1"]
...
[elsif exp="tf.dialog_save_overwrite"]
	...
	[eval exp="tf.dialog_text01 = 'スロット『'"]
	[eval exp="tf.dialog_text02 = tf.save_click_num+sf.saveload_now_page*20+1"]
...
[elsif exp="tf.dialog_load"]
	[eval exp="tf.dialog_text01 = 'スロット『'"]
	[eval exp="tf.dialog_text02 = tf.load_click_num+sf.saveload_now_page*20+1"]
	[eval exp="tf.dialog_text03 = '』のデータを'"]
...
;;メニューのタイトルに戻るボタンから　NO押した時の挙動がだめだな。面倒だから使わない
;[elsif exp="tf.dialog_menu_gotostart"]
;	[eval exp="tf.dialog_text01 = 'タイトルに戻りますか？'"]
;	[確認ダイアログ]
;	[eval exp="tf.dialog_loadtitle = 0"]
;	[locate x="&sf.dialog_YES_x" y="&sf.dialog_YES_y"][button graphic="dialog_bt_yes"                         target=*タイトルに戻る]
;	[locate x="&sf.dialog_NO_x"  y="&sf.dialog_NO_y"][button graphic="dialog_bt_no"                           target=*menuタイトルに戻らない]



;;×ボタンとかのゲーム終了　これだけうまくいかない……
;[elsif exp="tf.dialog_QUIT"]
;	[eval exp="tf.dialog_text01 = 'ゲームを終了しますか？'"]
;	[確認ダイアログ]
;	[eval exp="tf.dialog_QUIT = 0"]
;	[locate x="&sf.dialog_YES_x" y="&sf.dialog_YES_y"][button graphic="dialog_bt_yes" storage="dialog.ks" target=*Yesだよ]
;	[locate x="&sf.dialog_NO_x" y="&sf.dialog_NO_y"][button graphic="dialog_bt_no"    storage="dialog.ks" target=*Noだよ]
